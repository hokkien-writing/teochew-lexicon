...
|th| ① tho 胎 推   ② thó 討{讨} 橢{椭} 妥   ③ thò 蛻{蜕} 褪 退   ④ thoh 魄 托{托}(~舉) 託{托}(委~,拜~)   ⑤ thô 萄 桃 鴕{鸵} 陀 馱{驮}   ⑥ thõ 唾   ⑧ tho̍h 駝{驼}  |
...
|ch| ① chiu 稠 舟 州 洲 周{周}(圓~,~遊) 週{周}(~末,~期)   ② chiú 酒   ③ chiù 咒 皺{皱}   ⑥ chiũ 就  |
...
| | ① uai 挖 歪  |
|s| ① suai 甩  |
|k| ① kuai 乖 鮭{鲑}   ② kuái 拐{拐}(~騙) 柺{拐}(老人~)   ③ kuài 怪 劊{刽} 會{会} 繪{绘} 儈{侩} 檜{桧} 膾{脍}  |
|kh| ③ khuài 筷 快  |
...
|m| ② miáu 藐 秒 杳 渺(~茫) 淼{淼}(煙波浩~)   ⑤ miâu 錨{锚} 苗 描 瞄   ⑥ miãu 妙  |
|t| ① tiau 碉 叼 凋 刁 貂 雕{鵰}   ③ tiàu 吊{吊}(~燈) 弔{吊}(憑~)   ⑤ tiâu 條{条}   ⑥ tiãu 兆 召   ⑦ tiāu 掉 調{调}  |
|th| ① thiau 挑 迢   ② thiáu 眺 窕   ③ thiàu 超 跳   ⑤ thiâu 調{调}   ⑥ thiãu 柱  |
|n| ② niáu 溺 嫋{袅}   ③ niàu 皺{皱}  |
|l| ② liáu 了{了}(~斷) 瞭{了}(明~,~解)   ③ liàu 了(到~)   ⑤ liâu 撈{捞} 撩 聊 僚 療{疗} 燎 寥 遼{辽} 潦 鐐{镣} 獠 醪 瞭(~望)   ⑦ liāu 廖 料  |
...
|s| ① sian 仙 秈{籼}   ② sián 銑{铣} 洗 鮮{鲜} 癬{癣} 蘚{藓} 跣   ③ siàn 煽{煽}(~風點火) 搧{搧}(~面龐)   ④ siat 契 設{设} 楔 屑 褻{亵}   ⑤ siân 闡{阐} 單{单}   ⑥ siãn 單{单} 擅 膳 善 繕{缮} 腎{肾} 鱔{鳝} 撣{掸}(~族)  |
|j| ⑤ jiân 然 燃   ⑧ jia̍t 熱{热}  |
|k| ① kian 堅{坚} 捐 鵑{鹃} 娟   ③ kiàn 見{见} 建 絹{绢}   ④ kiat 吉 潔{洁}   ⑥ kiãn 鍵{键} 健 建 毽 犍   ⑧ kia̍t 傑{杰} 竭 桀  |
|kh| ① khian 虔   ② khián 遣 譴{谴} 犬   ④ khiat 撅 契 孑 挈 黠   ⑤ khiân 乾(~坤)  |
|ng| ② ngián 研   ③ ngiàn 願(～食)   ④ ngiat 齧{啮}   ⑤ ngiân 研 妍  |
...
|s| ① suan 喧 宣 萱   ② suán 選{选}   ③ suàn 氽 汆   ⑤ suân 旋{旋}(~轉,黑~風)   ⑥ suãn 旋(~尿)  |
...
|ts| ① tsang 贓{赃} 曾 莊{庄} 鬃 棕 臧 髒{脏}(骯~)   ② tsáng 總{总}   ③ tsàng 壯{壮} 糉{粽}   ④ tsak 節{节} 軋{轧} 鍘{铡} 作 扎{扎}(掙~) 紮{扎}(包~,駐~)   ⑤ tsâng 叢{丛}   ⑥ tsãng 藏 臟{脏}(~器)   ⑦ tsāng 贈{赠}  |
...
|p| ② péng 秉 炳   ④ pek 柏 薄 逼 伯 迫   ⑤ pêng 迸   ⑥ pẽng 幷{并}(~州) 併(合併) 並(並立,並且)  |
|ph| ① pheng 烹 拼(~凑)   ③ phèng 騁{骋} 聘   ④ phek 碧 霹 劈 魄 辟{辟}(復~) 闢{辟}(開~)   ⑤ phêng 砰 抨 硼 朋 坪 萍 平 憑{凭} 評{评} 怦  |
...
|ch| ① cheng 丁 睛 精 僧 增 憎 曾 貞{贞} 偵{侦} 蒸 掙{挣} 睜{睁} 猙{狰} 怔 幀{帧} 盅 甑 箏{筝} 舂 征(~服) 鐘{钟}(~頭) 鍾{钟}(~愛)   ② chéng 種{种} 腫{肿}   ③ chèng 蹭 正 政 證{证} 種{种} 衆{众} 症{症}(~狀)   ④ chek 叔 則{则} 燭{烛} 稷   ⑤ chêng 曾   ⑥ chẽng 靖 淨{净} 靚{靓}   ⑧ che̍k 蜀 一 壹 澤{泽}  |
|chh| ① chheng 稱{称} 清   ③ chhèng 襯{衬} 稱{称}   ④ chhek 側{侧} 測{测} 斥 僳 戚 粟 彳   ⑤ chhêng 橙 情   ⑦ chhēng 穿  |
|s| ① seng 聲{声} 生 甥 牲 勝{胜} 腥 猩 惺 旌 升{陞升}(~級,公~) 昇{升}(東~)   ② séng 省   ③ sèng 勝{胜} 聖{圣}   ④ sek 瑟 色 識{识} 式 適{适} 釋{释} 飾{饰} 碩{硕} 昔 析 矽 晰 息 悉 夕 熄 汐 媳 植 殖 擲{掷} 晳 皙 穡{穑} 窸 蟋   ⑤ sêng 成 乘 誠{诚} 承 丞 榕 松{松}(~樹)   ⑥ sẽng 乘 盛   ⑧ se̍k 熟  |
|j| ⑤ jêng 扔  |
|k| ① keng 更 供 宮{宫} 弓 荊{荆} 兢 驚{惊} 經{经} 矜   ② kéng 景 境 竟   ③ kèng 更 敬 亙{亘}   ④ kek 革 激 棘 菊 檄 亟   ⑤ kêng 瓊{琼}   ⑥ kẽng 警 競{竞}   ⑧ ke̍k 極{极} 局  |
|kh| ① kheng 兢 筐 卿   ② khéng 肯 啃   ③ khèng 慶{庆} 罄   ④ khek 擊{击} 刻 蛐 曲{曲}(戲~,~折) 麴{曲}(酒~)   ⑤ khêng 莖{茎} 鯨{鲸} 痙{痉} 擎 扃  |
|g| ⑧ ge̍k 玉 獄{狱}  |
|ng| ⑤ ngêng 凝(~固) 迎   ⑧ nge̍k 逆  |
|h| ① heng 哼 亨 興{兴} 胸 匈 馨   ③ hèng 興{兴}   ④ hek 核 閡{阂} 黑   ⑤ hêng 恆{恒} 王 刑 型 形 行 雄   ⑥ hẽng 行 杏 孕 幸{幸}(~災樂禍,寵~) 倖{幸}(僥~,~存)   ⑧ he̍k 乏  |
...
|ph| ② phóng 捧 搿   ③ phòng 膀 碰   ④ phok 爆 撲{扑} 仆{仆}(前~後繼) 朴{朴}(~樹) 樸{朴}(~素)   ⑤ phông 縫{缝} 蓬 篷 鵬{鹏}  |
|m| ② móng 某 晚   ③ mòng 貿{贸}   ⑤ mông 芒 檬 摸 謀{谋} 蠓 蒙(~古,啟~,~難,~娜麗莎) 濛{蒙}(~煙) 矇{矇}(~矓,~騙) 朦{朦}(~朧)   ⑥ mõng 茂 舞   ⑧ mo̍k 沒{没} 莫 漠 寞 木 睦 牧 穆 疫 沐  |
|t| ① tong 東{东} 冬 中 忠   ② tóng 董 懂   ③ tòng 棟{栋} 凍{冻} 疼 中   ④ tok 督 築{筑} 斫{斲斫}(~斷)   ⑤ tông 幢   ⑥ tõng 動{动} 重 仲   ⑧ to̍k 毒 犢{犊} 獨{独} 逐 篤{笃}  |
...
|s| ① song 鬆{松}(輕~)   ② sóng 聳{耸} 慫{怂}   ③ sòng 宋   ④ sok 寂 叔 淑 束 數{数} 漱 速 塑 肅{肃} 縮{缩} 索 夙   ⑤ sông 嵩 松{松}(~樹)   ⑥ sõng 頌{颂} 訟{讼} 誦{诵}   ⑧ so̍k 贖{赎} 孰 屬{属} 俗 續{续}  |
...
|h| ① hong 封 峯{峰} 鋒{锋} 風{风} 烽 轟{轰} 烘 丰(~采) 豐{丰}(~盛) 哄(~堂大笑)   ② hóng 蚌 俸 哄{哄}(~騙)   ③ hòng 諷{讽} 訌{讧} 哄{哄}(~搶) 鬨{哄}(起~)   ④ hok 輻{辐} 福 複{复}(重~,~雜)   ⑤ hông 逢 縫{缝} 烘 虹 鴻{鸿} 洪 宏 弘 紅{红} 肱   ⑥ hõng 奉 鳳{凤}   ⑧ ho̍k 伏 服 袱 覆{复}(反~,~蓋) 或 惑 域 復{复}(~仇,~復)  |
...
| |iong / iok|
|-|----|
| | ① iong 臃 癰{痈} 雍   ② ióng 踴{踊} 蛹 永 勇   ③ iòng 擁{拥} 詠{咏} 泳 恿 湧{涌涌}(~現)   ⑤ iông 蓉 榮{荣} 融 熔 溶 容 瑩{莹} 螢{萤} 熒{荧} 盈 庸 榕 縈{萦} 佣(~金) 傭{佣}(~人)   ⑧ io̍k 峪 育 浴 欲{欲}(暢所~言,搖搖~墜) 慾{欲}(~望)  |
|th| ④ thiok 蓄 畜  |
|k| ① kiong 恭 躬 共 穹   ② kióng 拱   ④ kiok 鞠 粥  |
|kh| ② khióng 鞏{巩} 恐   ④ khiok 可 克(~服,千~) 剋{克}(~扣,相~)   ⑤ khiông 窮{穷}  |
|h| ① hiong 匈 洶{汹} 凶{凶}(~惡,~~) 兇{凶}(~手)   ④ hiok 旭 毓 馥 郁(文采~~,~達夫)   ⑤ hiông 雄  |
...
|h| ① huang 坊 芳 方 肪 封 風{风} 瘋{疯} 荒 慌 謊{谎}   ② huáng 坊 訪{访} 紡{纺} 晃 幌 恍 仿(模~) 彷{仿}(~彿)   ③ huàng 放   ⑤ huâng 防 妨 衡 磺 蝗 簧 皇 凰 惶 煌 徨  |
...
|k| ① kng 缸 崗{岗} 光 扛   ② kńg 耿 管 廣{广} 卷{卷}(試~)   ③ kǹg 鋼{钢}  |
|kh| ① khng 糠   ③ khǹg 勸{劝}  |
